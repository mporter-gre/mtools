...
for thisImage = 1:numImages
    ...
    dataOut = [dataOut; {'File Name', 'Dataset', 'Timestamp', 'Frap Intensities' 'Ref Intensities', 'Base Intensities', 'Whole Intensities', 'Frap Normalised Corrected';}];
    for thisTimestamp = 1:length(roiShapes{thisImage}{indices{thisImage}.frapIdx}.correctT)
        dataOut = [dataOut; {roiShapes{thisImage}{indices{thisImage}.frapIdx}.name datasetNames{thisImage} num2str(roiShapes{thisImage}{indices{thisImage}.frapIdx}.timestamp(thisTimestamp))} roiShapes{thisImage}{indices{thisImage}.frapIdx}.frapData(thisTimestamp) roiShapes{thisImage}{indices{thisImage}.refIdx}.refData(thisTimestamp) roiShapes{thisImage}{indices{thisImage}.baseIdx}.baseData(thisTimestamp) roiShapes{thisImage}{indices{thisImage}.wholeIdx}.wholeData(thisTimestamp) roiShapes{thisImage}{indices{thisImage}.frapIdx}.frapNormCorr(thisTimestamp)];
    end
    dataOut = [dataOut; {' ',' ',' ',' ',' ',' ',' ',' '}];
